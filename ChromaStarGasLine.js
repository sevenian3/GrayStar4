...
// 
  ...
  var masterTemp = [130, 500, 3000, 8000, 10000];
...
// Convert to natural logs:
       ...
       //Get the temperatures corresponding to the tabulated partition function data:
          var pfT = getPartFnT2();
          //console.log("pfT " + pfT);
...
//NEW Determine temperature dependent partition functions Uw: lburns
        ...
        thisLogUw = interpol(pfT, logUw, thisTemp);

//Careful about the ends of the table:
        if (thisTemp >= 10000){
            thisLogUw = logUw[4];
        }
        if (thisTemp <= 130){
...
//This version - ionization equilibrium *WITHOUT* molecules - logNum is TOTAL element population
    var stagePops = function(logNum, Ne, chiIArr, logUw,  //species A data - ionization equilibrium of A
                 numDeps, temp) {
...
//atomic ionization stage Boltzmann factors:
        ...
        for (var i = 0; i < numStages; i++){
           logChiI = Math.log(chiIArr[i]) + logEv;
...
//   For atomic ionization stages:
        ...
        for (var i = 0; i < numStages+1; i++){
           logSaha[i] = [];
           logSaha[i].length = numStages+1;
        }
        var saha = [];
        saha.length = numStages+1;
        for (var i = 0; i < numStages+1; i++){
           saha[i] = [];
           saha[i].length = numStages+1;
        }
//
        var logIonFrac = [];
...
// for molecule formation:
        var logUwA = [];
        logUwA.length = 5;
        for (var kk = 0; kk < logUwA.length; kk++){
...
// NEW Determine temperature dependent partition functions Uw: lburns
       ...
       for (var iStg = 0; iStg < numStages; iStg++){
          thisLogUw[iStg] = interpol(masterTemp, logUw[iStg], thisTemp);
       }
...
// NEW Determine temperature dependent partition functions Uw: lburns
            for (var iStg = 0; iStg < numStages; iStg++){
              thisLogUw[iStg] = interpol(pfT, logUw[iStg], thisTemp);
            }
            for (var iMol = 0; iMol < numMols; iMol++){
              thisLogUwB[iMol] = interpol(pfT, logUwB[iMol], thisTemp);
            }
//Careful about the ends of the tables:
        if (thisTemp <= 130){
            for (var iStg = 0; iStg < numStages; iStg++){
                thisLogUw[iStg] = logUw[iStg][0];
            }
...
// 
    ...
    var stagePops3 = function(logNum, Ne, chiIArr, logUw,  //species A data - ionization equilibrium of A
                 numDeps, temp) {
...
// for molecule formation:
   ...
   //Ionization stage Saha factors:
            ...
            for (var iStg = 1; iStg < numStages; iStg++){
...
// NEW Determine temperature dependent partition functions Uw: lburns
        thisTemp = temp[0];
        thisLogUwU = interpol(pfT, logUwU, thisTemp);
        thisLogUwL = interpol(pfT, logUwL, thisTemp);
//Careful about the ends of the tables:
        if (thisTemp <= 130){
            thisLogUwU = logUwU[0];
            thisLogUwL = logUwL[0];
...
// NEW Determine temperature dependent partition functions Uw: lburns
        thisLogUwA = interpol(pfT, logUwA, thisTemp);
        nmrtrThisLogUwB = interpol(pfT, nmrtrLogUwB, thisTemp);
        for (var iMol = 0; iMol < numMolsB; iMol++){
            thisLogUwB[iMol] = interpol(pfT, logUwB[iMol], thisTemp);
        }
//Careful about the ends of the tables:
